...
export async function updateUserEarnings(userId: string) {
  ...
  // Rate limit: Don't update more than once every 5 minutes per user
  // This drastically improves performance of stats API
  // We can use a simple memory cache or check updatedAt
  if (user.updatedAt && (now.getTime() - user.updatedAt.getTime() < 300000)) {
     return;
  }
  ...
  for (const inv of user.investments) {
    ...
    if (daysSinceLastUpdate >= 1) {
      ...
      if (isCycleEnded && inv.autoReinvest) {
        ...
        const newEndDate = addDays(newStartDate, 35);
